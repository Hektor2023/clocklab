...
void RTCSystemTimeHandler::forceUpdateTime( void)
{
  ...
  MyTime  time;
  time.setSecond( rtc.second()); time.setMinute( rtc.minute()); time.setHour( rtc.hour());
  systemTimestamp.setTime( time);

  MyDate  date;
  date.setDay( rtc.day()); date.setMonth( rtc.month()); date.setYear( 2000 + rtc.year());
